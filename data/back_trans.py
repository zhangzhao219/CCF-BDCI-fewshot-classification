...
# python3 trans.py --filefolder ./ --filename train_de --src_language zh-cn --dst_language de --begin_line 0
    ...
    if idx >= args.begin_line:
        while True:
            try:
                ...
                row['title'] = translator.translate(row['title'],src=args.dst_language,dest=args.src_language).text
                row['assignee'] = translator.translate(row['assignee'],src=args.dst_language,dest=args.src_language).text
                row['abstract'] = translator.translate(row['abstract'],src=args.dst_language,dest=args.src_language).text
                data.iloc[idx] = row
                break
            except:
                print(idx," Retrying")
