...
python main.py \
--train \
--batch 12 --board --datetime ${TIMESTAMP} --epoch 40 --gpu 2 3 --lr 2e-5 --seed ${SEED} \
--data_folder_dir fewshot --data_file expand_train.json --label 36 \
--checkpoint 20 --save \
--bert ${BERT} --dropout 0.3 \
--K ${K} --split_test_ratio 0.2 --warmup 0.1
# --ema 0.999 --fgm --pgd 3 --rdrop 0.4 --warmup 0.1
  
## 测试
python main.py \
--test \
--batch 128 --datetime ${TIMESTAMP} --gpu 2 3 --seed ${SEED} \
--data_folder_dir fewshot --data_file expand_train.json --label 36 \
--bert ${BERT} --dropout 0.3 \
--K ${K}
