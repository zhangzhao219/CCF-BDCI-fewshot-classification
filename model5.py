...
class PretrainedModel(nn.Module):
    ...
    def forward(self,input_ids,token_type_ids,attention_mask):
        ...
        output_last = torch.cat([outputs['hidden_states'][-i][:, 0] for i in range(1, self.feature_layers+1)], dim=-1)
        output_first = torch.cat([outputs['hidden_states'][i][:, 0] for i in range(0, self.feature_layers)], dim=-1)
        output = (output_last + output_first) / 2
